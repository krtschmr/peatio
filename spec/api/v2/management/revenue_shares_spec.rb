...
describe API::V2::Management::RevenueShares, type: :request do
  ...
  describe 'list revenue shares' do
    ...
    it 'returns revenue shares' do
      ...
      expect(JSON.parse(response.body).map { |x| x.fetch('id') }).to eq RevenueShare.order(id: :asc).pluck(:id)
    end
  ...
  describe 'update revenue share' do
    ...
    let(:percent) { 0.5 }
    let(:signers) { %i[alex jeff] }
    let(:record) { RevenueShare.create!(member: member, parent: parent, percent: percent) }
    let(:data) { { id: record.id } }

    it 'validates revenue share id' do
      data.merge!(id: RevenueShare.last.id + 1)
      request
      expect(response).to have_http_status(404)
    end

    it 'updates revenue share state' do
      data.merge!(state: 'disabled')
      request
